 <?php 
    ...
    /*require('../adminunicab/PHPMailer_master/src/Exception.php');
    require('../adminunicab/PHPMailer_master/src/PHPMailer.php');
    require('../adminunicab/PHPMailer_master/src/SMTP.php');
...
<?php
	...
	$contenido = $_REQUEST['editor'];
	$contenido1 = str_replace("&lt;", "<", $contenido);
	$contenido2 = str_replace("&gt;", ">", $contenido1);
	echo $contenido2;
	
	try {	
		$sql_blog="INSERT INTO noticia (DescripcionN) 
		VALUES ('".$contenido."')";
		$exe_blog=mysqli_query($conexion,$sql_blog);
		echo $sql_blog;
		
		//Se consulta el id creado
		$sql_id = "SELECT Max(IdNoticia) maxIdNoticia FROM noticia";
		echo "<br>".$sql_id;
		$exe_id = mysqli_query($conexion,$sql_id);

		while ($fila = mysqli_fetch_array($exe_id)){
						  
			$id_noticia = $fila['maxIdNoticia'];
		}
		echo "<br>id_noticia: ".$id_noticia;
		
		$sql_upd = "UPDATE noticia SET DescripcionN = REPLACE(DescripcionN, '&lt;', '<') WHERE IdNoticia = $id_noticia";
		echo "<br>".$sql_upd;
		$exe_upd = mysqli_query($conexion,$sql_upd);
		
		$sql_upd1 = "UPDATE noticia SET DescripcionN = REPLACE(DescripcionN, '&gt;', '>') WHERE IdNoticia = $id_noticia";
		echo "<br>".$sql_upd1;
