<?php
	...
	$nomconv=str_replace("_"," ",$_REQUEST['txtnomconvp']);
	$idconv=$_REQUEST['txtidconvp'];
	$pc=$_REQUEST['txtpcp'];
	//echo $nomconv;
	//echo $idconv;
	//echo $pc;
	...
	$a=$_REQUEST['anomes'];
	$estado=$_REQUEST['selest2'];
	//$per=$_REQUEST['anomes'];
	$tipo=$_REQUEST['seltranp'];
	//echo $tipo;
	...
	if(!$p) {
		...
		header("Location: tranest_getdat1.php?p=1");
	}
	
	//Se consultan las transacciones
	if($tipo == "TC") {
	    $sql = "SELECT t.*, 'TC' tipo, p.concepto FROM tbl_transac_tc t, tbl_pagos p 
	    WHERE t.cod_ref_pago = p.codigo AND convenio = '$nomconv' AND substring(fecha_upd,1,6) = '$a'";
	}
	else if($tipo == "PSE") {
