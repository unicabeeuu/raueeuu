<?php
	...
	set_time_limit(300);
	//https://unicab.org/registro/docenteunicab/updreg/estudianteg_getdat_prueba.php?selgra1=15&estadog=activo
...
<html>
		...
		<script>
            ...
            $(function() {
                ...
                $("#tblest tbody tr").click(function(){ 
                    ...
                    let value1=$(this).find('td:nth-child(3)').text();
                    let value2=$(this).find('td:nth-child(4)').text();
                    $("#txtidest").val(value);
                    $("#txtidgra").val(value1);
                    $("#txtgra").val(value2);
            ...
            function ver_perdiendo(id_gra) {
                //alert("ingreso a ver perdiendo");
                
            	$.ajax({
            		type:"POST",
            		url:"perdiendo_por_pens.php",
            		data:"idgra=" + id_gra,
            		success:function(r) {
            			//mostrar_notas(r);
            			mostrar_perdiendo(r);
            ...
            //*******************************************************************************
            function mostrar_perdiendo(data) {
                //alert("ingreso a mostrar perdiendo");
                
                let grado = $("#txtgra").val();
                
                let res = JSON.parse(data);
                
                if(res.lbls.length == 6) {
                    let datos = {
    			...
    			$("#graficop").remove();
    			//Se genera un nuevo canvas
    			$("#divcanvas_perd").append('<canvas id="graficop" width="800" height="350"></canvas>');
                
                let canvas1 = document.getElementById("graficop").getContext("2d");
                window.bar = new Chart(canvas1, {
                    ...
                    options : {
                        ...
                        title : {
                            display : true,
                            text : "TOTAL ESTUDIANTES PERDIENDO POR PERIODO GRADO " + grado,
                            fontSize : 14
                        },
                        legend : {
                            display : true,
                ...
                //para escalas ver https://www.youtube.com/watch?v=_vRS87AT1Yk (13:20)
                //window.bar.update();
            }
            //*******************************************************************************
	...
	<body>
		<center>
				...
				<table>
					<tbody>
						...
						<tr>
							...
							<td>
							    <!--<button class='btn btn-warning glyphicon glyphicon-list-alt' data-toggle='modal' data-target='#modal_est_perd' title='Datos'></button>-->
							    <button class='btn btn-danger' data-toggle='modal' data-target='.bd-example-modal-lg'><span class="fa fa-thumbs-down" aria-hidden="true"></span> Total estudiantes perdiendo por periodo</button>
							    </br></br>
			...
			<!-- Total estudiantes perdiendo por periodo 
			<div id="divcanvas_perd" style="display: block;">
			    <canvas id="graficop" width="800" height="200"></canvas>
			</div>-->
		</center>
		
		<!-- Modal total estudiantes perdiendo por periodo -->
        <div class="modal fade bd-example-modal-lg" id="modal_est_perd" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
              ...
              <div class="modal-body">
                <!-- Total estudiantes perdiendo por periodo -->
                <div id="divcanvas_perd" style="display: block;">
                    <canvas id="graficop" width="800" height="300"></canvas>
                </div>
              </div>
