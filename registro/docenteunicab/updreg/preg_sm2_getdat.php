<?php
	...
	set_time_limit(300);
	//https://unicab.org/registro/docenteunicab/updreg/preg_sm2_getdat.php?idgra=10&idpen=5
	...
	WHERE p.id_tema = tp.id AND p.id_grado = $idgra AND p.id_materia = $idpen AND p.id_tipo_pregunta = 4";
	//echo $query1;
	...
	while($row = $resultado->fetch_assoc()) {
	    $cadena = $cadena."<tr>
                ...
                onclick='enviardat(".$row['id'].",".$row['id_tema'].",\"".$row['pregunta']."\",\"".$row['r1ok']."\",\"".$row['retroalimentacion']."\",\"".$row['imagen']."\",\"".$row['r1no']."\",\"".$row['r2no']."\",\"".$row['r2ok']."\")'></button></td></tr>";
