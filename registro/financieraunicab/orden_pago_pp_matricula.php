<?php
    ...
    //https://unicab.org/registro/financieraunicab/orden_pago_pp_matricula.php?register_grado=10&register_documento=999999&register_nombres=GREGORY&register_apellidos=FIGUEREDO&register_tipo_documento=3&register_telefono=11&register_email=g@g.com&estnuevo=NO&td_text=CEDULA&register_documentoA=9397454&register_nombresA=HERNANDO&register_apellidosA=GUEVARA&register_direccionA=CALLE&register_celularA=22&register_correoA=g.h.fig.1073@gmail.com
	...
	$sql_beca = "SELECT * FROM tbl_becas WHERE identificacion = $documento AND periodo_lectivo = 2021";
    ...
    while($row = $resultado->fetch_assoc()){
        ...
        $content .= '.crojo {color: red; font-weight: bold; font-size: 11px; text-align: justify;}';
        ...
        $content1 .= '<div>';
            ...
            $content1 .= '<div id="divbody0">';
                $content1 .= '<table>';
                    ...
                    $content1 .= '<tr>';
                            ...
                            //************************** Se valida si es matrícula ordinaria **************************
                            $sql_matord = "SELECT f1 FROM tbl_parametros WHERE parametro = 'mat_ordinarias'";
                            $exe_matord = $mysqli1->query($sql_matord);
                            while($row_matord = $exe_matord->fetch_assoc()) {
                                $f_matord = $row_matord['f1'];
                            }
                            if($fecha3 > $f_matord) {
                                //Se valida si el estudiante es nuevo y ya está programado con entrevista
                                $sql_premat = "SELECT COUNT(1) ct FROM tbl_pre_matricula WHERE documento_est = '$documento' AND id < 1249"; 
                                $exe_premat = $mysqli1->query($sql_premat);
                                while($row_premat = $exe_premat->fetch_assoc()) {
                            ...
                            //*************************** FIN valida si es matrícula ordinaria **************************
                            if($mat_extra == "SI") {
                                $valor = $valor + $valor * $inc_mat;
                            }
            ...
            if($mat_extra == "SI") {
                $content1 .= '<p id="lblconvenciones" class="cpurple">* Primer pago (pp) => Matrícula (m) {'.$row['matricula'].'} + pensión mes 1 (pm1) {'.$row['pension_final'].'} + otros cobros periódicos (ocp) {'.$row['ocp'].'} + poliza {'.$row['poliza'].'} + 3% de incremento de la suma de los items anteriores por matrícula extraordinaria.</p>';
            }
            else {
