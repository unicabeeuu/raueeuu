<?php
    ...
    //https://unicab.org/registro/financieraunicab/orden_pago_codbar.php?selgra1=10&idest=1014859175
    ...
    if ($documento == 0) {
        ...
        AND e.n_documento NOT IN (SELECT identificacion FROM tbl_pagaron_todo WHERE a = $fanio)";
    }
    else {
        $query = "SELECT e.*, m.id_grado, cu.matricula, cu.pension, cu.ocp, cu.poliza, cu.dg, cu.pp, eg.grado_ra 
        FROM estudiantes e, matricula m, tbl_costos_unicab cu, equivalence_idgra eg 
        WHERE e.id = m.id_estudiante AND m.id_grado = cu.id_grado AND m.id_grado = eg.id_grado_ra 
        AND m.estado IN ('activo', 'solicitud') AND cu.a = $fanio AND m.id_grado = $idgra AND e.n_documento = $documento";
    }
    //echo $query;
    ...
    while($row = $resultado->fetch_assoc()){
        ...
        for($i=0; $i<$longitud_f; $i++) {
            ...
            $content .= '.thfondo {font-weight: bold; background: lihghtblue; color: black;}';
                            ...
                            $content1 .= '<tbody>';
                                $content1 .= '<tr>';
                                    ...
                                    $content1 .= '<td class="tdop">';
                                        $content1 .= '<h4>'.$row['documento_responsable'].'_'.$fanio.'_'.$pagos_f[$i].'_a</h4>';
                                    $content1 .= '</td>';
                        ...
                        $content1 .= '<table>';
                            $content1 .= '<tbody>';
                                $content1 .= '<tr>';
                                    ...
                                    $content1 .= '</td>';
                                    $content1 .= '<td class="tdop">';
                                        $content1 .= '<h4>'.$row['n_documento'].'_'.$fanio.'_'.$pagos_f[$i].'</h4>';
                                    $content1 .= '</td>';
                                    $content1 .= '<td class="tdsinborde2"><h4>Fecha de expedición: '.$fecha2.'</h4></td>';
                                $content1 .= '</tr>';
                            $content1 .= '</tbody>';
            ...
            /*$content1 .= '<br><hr><img src="barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code39&print=true&sizefactor=2"/><hr>
            <img src="barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code128&print=true&sizefactor=2"/><hr>
            <img src="barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code39&print=true&sizefactor=1"/><hr>
            <img src="barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code128&print=true&sizefactor=1"/><hr>';*/
            $content1 .= '<br><hr><img src="barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code39&print=true&sizefactor=1"/><hr>
            <img src="barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code128&print=true&sizefactor=1"/><hr>';
            $content1 .= '</div>';
            
            $barcode = file_get_contents('https://unicab.org/registro/financieraunicab/barcode.php?text='.$row['n_documento'].'|'.$number.'|'.$fanio.'_'.$pagos_f[$i].'|20201210&size=50&codetype=code128&print=true&sizefactor=1');
            //save the file to disk 
            file_put_contents('img/codebar1.png', $barcode);
            
            //Este archivo se genero con prueba_barcode.php en updreg
            $content1 .= '<div><img src="phpqrcode/img/qr2.png" width="100px"/></div>';
            
            $content1 .= '</body></html>';
            
            $nom_pdf = "op_cb_".$row['grado_ra']."_".str_replace(" ","_",$nombrecompleto)."_".$fanio.".pdf";
        ...
        //echo $ruta;
        echo '<a href="'.$ruta.'" target="_blank">DESCARGAR</a>';
        ...
        try {
            ...
            $mail->Body    = '<p>Señor(a): '.strtoupper($row['acudiente_1']).'</p>
                ...
                <p>También le recomendamos hacer el pago del mes 10 y los derechos de grado (o del mes 5 y los derechos de grado para Ciclo IV y Ciclo VI), en un solo pago. Encontrará ésta orden al final del documento para los grados quinto, noveno, once, ciclo IV y ciclo VI.</p>
                <br><p>--</p>
                <p>Área Financiera</p>
...
<html>
    ...
    <body>
        ...
        <hr><img src="barcode.php?text=9397454|180000|pp2020|20201210&size=50&codetype=code128&print=true&sizefactor=2"/><hr>
        <img src="barcode.php?text=9397454|180000|pp2020|20201210&size=50&codetype=code39&print=true&sizefactor=2"/><hr>
        <img src="barcode.php?text=9397454|180000|pp2020|20201210&size=50&codetype=code128&print=true&sizefactor=1"/><hr>
        <img src="barcode.php?text=9397454|180000|pp2020|20201210&size=50&codetype=code39&print=true&sizefactor=1"/>
    </body>
</html>
