...
if (isset($_SESSION['unisuper']) || isset($_SESSION['uniprofe'])) {
	...
	$sql_num = "SELECT DISTINCT en.documento_est, pm.nombre_a, pm.celular_a, '0' telefono_acudiente_1, pm.nombres_est, pm.apellidos_est, 'NA' n_matricula, 'NA' estado, LENGTH(pm.celular_a) 
    FROM tbl_entrevistas en, tbl_pre_matricula pm 
    WHERE en.documento_est = pm.documento_est AND en.id >= 1892 AND LENGTH(pm.celular_a) = 10 AND pm.entrevista = 'SI' 
    AND en.documento_est NOT IN (SELECT DISTINCT en.documento_est 
    FROM tbl_entrevistas en, tbl_pre_matricula pm, estudiantes e, matricula m 
    WHERE en.documento_est = pm.documento_est AND en.documento_est = e.n_documento AND e.id = m.id_estudiante AND en.id >= 1892 
    AND m.n_matricula like '%2023%' AND m.estado = 'pre_solicitud')";
    
    /*$sql_num = "SELECT nombre_a, celular_a, nombres_est, apellidos_est FROM tbl_pre_matricula WHERE documento_est = '1111'";*/
        ...
        if($destino == $fila_num['telefono_acudiente_1']) {
        	...
        	if($envio == 1) {
                ...
                curl_setopt_array($curl, array(
                  ...
                  CURLOPT_POSTFIELDS => "token=".$token."&to=+57".$fila_num['celular_a']."&image=".$rutaimage."&caption=".$textoimg."&referenceId=",
                ...
                curl_setopt_array($curl, array(
                  ...
                  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                  CURLOPT_CUSTOMREQUEST => "POST",
                  CURLOPT_POSTFIELDS => "token=".$token."&to=+57".$fila_num['celular_a']."&body=".$textowhat."&priority=1&referenceId=",
                  CURLOPT_HTTPHEADER => array(
                    "content-type: application/x-www-form-urlencoded"
                  ),
                ));
            ...
            //$keys = ['response','acudiente','celular'];
            $valores = [$response,$fila_num['nombres_est']." ".$fila_num['apellidos_est'],$fila_num['celular_a']];
				...
				<div class="forms">
					<div class="form-grids row widget-shadow" data-example-id="basic-forms"> 
						<div class="form-title">
							<h4>Envío masivo de whatsapp entrevistas sin proceso de matrícula:</h4>
						</div>
						<div class="form-body">
							<?php
							    echo "url: ".$url."<br>";
