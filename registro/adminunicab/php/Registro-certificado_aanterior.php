<?php 
    ...
    /*$peticion= "SELECT e.nombres, e.apellidos, e.genero, td.tipo_documento, e.n_documento, e.expedicion, e.ciudad, g.grado, g.id, 
        ...
        AND date_format(fecha_ingreso, '%Y') = $aant)) m ON e.id = m.id_estudiante 
    ...
    $peticion= "SELECT e.nombres, e.apellidos, e.genero, td.tipo_documento, e.n_documento, e.expedicion, e.ciudad, g.grado, g.id, 
        ...
        (SELECT * FROM matricula WHERE idMatricula = 
        (SELECT MAX(idMatricula) maxid FROM matricula WHERE id_estudiante = ".$id_estudiante." AND estado IN ('aprobado', 'reprobado', 'activo') 
        AND n_matricula like '%".$aant."%')) m ON e.id = m.id_estudiante 
        INNER JOIN grados g ON m.id_grado=g.id INNER JOIN tbl_tipos_documento td ON e.tipo_documento = td.id 
        WHERE m.id_estudiante=".$id_estudiante." and m.n_matricula = '".$matricula_actual."'";
    //echo $peticion;
	...
	if ($idioma=="espanol") {
		...
		$content1 .= '<p style="border: black 2px solid; text-align: left;  border-width: thin; width:30%; font-family: sans-serif; font-size: 1rem;"><strong>'.$mensaje_promovido.'</strong></p>';
        ...
        $content1 .= '<img src="../../images/firma_certificados.JPG" width="700" height="166"/>';
        //$content2 .= '<br><br><br><img src="../../images/sin_firmas1.jpg" width="495px" height="55px"/>';
        $content2 .= '<br><br><br><img src="../../images/firma_certificados_liliana_sf.jpg" width="700px" height="63px"/>';
        ...
        if($ct_pdf > 0) {
            ...
            $sql_insert='INSERT INTO certificado (fecha_expedicion, numero, tipo_certificado, id_estudiante, id_grado, ruta, numero1, identificacion, ruta1) 
            VALUES ("'.$fecha2.'","CFF'.$aant.$numeroc.'","Certificado final","'.$id_estudiante.'","'.$id_grado.'","'.$ruta.'",'.$aant.$numeroc.',"'.$n_documento.'","'.$ruta1.'")';
            //echo $sql_insert;
	...
	if ($idioma=="ingles") {
		...
		$content1 .= '</tbody></table><br>';
		$content1 .= '<div style="text-align: left;">';
		$content1 .= '<p style="border: black 2px solid; text-align: left;  border-width: thin; width:30%; font-family: sans-serif; font-size: 1rem;"><strong>'.$mensaje_promovidoIngles.'</strong></p>';
		$content1 .= '</div></div></center>';
        ...
        if($ct_pdf > 0) {
            ...
            $sql_insert='INSERT INTO certificado (fecha_expedicion, numero, tipo_certificado, id_estudiante, id_grado, ruta, numero1, identificacion, ruta1) 
            VALUES ("'.$fecha2.'","CFIF'.$aant.$numeroc.'","Certificado final","'.$id_estudiante.'","'.$id_grado.'","'.$ruta.'",'.$aant.$numeroc.',"'.$n_documento.'","'.$ruta1.'")';
            //echo $sql_insert;
            $exe_insert=mysqli_query($conexion,$sql_insert);
        }
	...
	/*$sql_insert='INSERT INTO `certificado`(`fecha_expedicion`, `numero`, `tipo_certificado`, `id_estudiante`, `id_grado`) 
    VALUES ("'.$fecha_expedicion.'","'.$numeroc.'","'.$tipo_certificado.'","'.$id_estudiante.'","'.$id_grado.'")';
    $exe_insert=mysqli_query($conexion,$sql_insert);*/
    //echo"<script>alert('ghf')</script>";
    //echo "<script>alert('Certificado Generado con Éxito')</script>";
?>
