...
if (isset($_POST)) {
	...
	$secret ="6Lc6bVYUAAAAAKacD2jtl3-U71hljlBJEou4JdWe";
		if (!$captcha) {
			...
			echo "<script>location.href='../../docenteunicab/login.php';</script>";
			...
			if($arr['success']){
				//$sql="SELECT * FROM profesores where email_institucional='".$usuario."' and password='".$password."'";
				$sql="SELECT * FROM tbl_empleados where email='".$usuario."' and pc='".$password."' AND perfil IN ('SU','TU','TU_AR_AW','TU_AW')";
				$petecion=mysqli_query($conexion,$sql);
				$fila=mysqli_num_rows($petecion);
