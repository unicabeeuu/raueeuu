...
if (isset($_SESSION['unisuper']) || isset($_SESSION['uniprofe'])) {
    ...
    //Se consulta la cantidad de contactos drive
    /*$sql_cont = "SELECT COUNT(1) ct FROM 
    (SELECT DISTINCT telefono_acudiente_1 FROM tbl_prueba WHERE texto = 'NA') a";*/
	$sql_cont = "SELECT COUNT(1) ct, m.id_grado, g.grado FROM tbl_prueba p, estudiantes e, matricula m, grados g 
	WHERE (p.telefono_acudiente_1 = e.telefono_acudiente_1 OR p.telefono_acudiente_1 = e.telefono_acudiente_2) 
	AND e.id = m.id_estudiante AND m.id_grado = g.id AND p.texto = 'NA' GROUP BY m.id_grado, g.grado;";
    
    $res_cont=mysqli_query($conexion,$sql_cont);
    ...
    function validar_datos() {
        ...
        if(control == 0) {
            ...
            if(ident != 0) {
				...
				$("#btnguardar").hide();
                alert("Debe seleccionar Contactos Drive");
            }
        }
								...
								<div class="form-group"> 
									...
									<table border="1px" style="text-align: center;">
									    ...
									    <tbody>
									        <?php
									            $cadena = "";
									            while($row_ant = $res_cont->fetch_assoc()) {
                                            	    $cadena = $cadena."<tr>
                                                        <td>".$row_ant['id_grado']."</td>
                                                        <td>".$row_ant['grado']."</td>
                                                        <td>".$row_ant['ct']."</td>
                                                        <td style='text-align: center;'><input type='radio' id='rd".$row_ant['id_grado']."' name='rdentre' class='chk' value='".$row_ant['id_grado']."' onchange='marcarident(this.value);'></td>
                                                        </tr>";
                                            	    echo $cadena;
                                            	    $cadena = "";
                                            	}
