...
if (isset($_SESSION['unisuper']) || isset($_SESSION['uniprofe'])) {
    ...
    //Se consulta la cantidad de pre_solicitudes
    $sql_pre = "SELECT COUNT(1) ct FROM 
    (SELECT e.n_documento, e.acudiente_1, e.telefono_acudiente_1, e.nombres, e.apellidos, m.estado 
    FROM estudiantes e, matricula m 
    WHERE e.id = m.id_estudiante AND m.estado = 'pre_solicitud' AND m.n_matricula like '%2023%') a";
    
    $res_pre=mysqli_query($conexion,$sql_pre);
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('input[name="rdpre"]').prop('checked', false);
        $("#txtidpre").val("0");
