...
if (isset($_SESSION['unisuper']) || isset($_SESSION['uniprofe'])) {
	...
	$vidservidor = $_REQUEST['selvid'];
	$textowhat = $_REQUEST['textoW'];
	
	$textoimg = $_REQUEST['textoI'];
	$idmat = $_REQUEST['txtidmat'];
	//echo $idmat;
	
	$textopdf = $_REQUEST['textoPdf'];
	...
	//if($idmat == "1") {
	if($idmat != "1") {
	    $sql_num = "SELECT e.acudiente_1, e.telefono_acudiente_1 
        FROM estudiantes e, matricula m 
        WHERE e.id = m.id_estudiante AND m.estado = 'activo' AND m.n_matricula like '%2023%' AND e.id NOT IN (1040, 1155) 
		AND m.id_grado = ".$idmat." 
        ORDER BY e.telefono_acudiente_1";
	}
	else if($idmat == "2999") {
	    $sql_num = "SELECT e.acudiente_1, e.telefono_acudiente_1 
        FROM estudiantes e, matricula m 
        WHERE e.id = m.id_estudiante AND m.estado = 'solicitud' AND m.n_matricula like '%2023%' AND e.id NOT IN (1040, 1155) 
        ORDER BY e.telefono_acudiente_1";
	}
	else {
		$sql_num = "SELECT e.acudiente_1, e.telefono_acudiente_1 
		FROM estudiantes e, matricula m 
		WHERE e.id = m.id_estudiante AND m.estado = 'activo' AND m.n_matricula like '%2023%' AND e.id NOT IN (1040, 1155) 
		AND e.telefono_acudiente_1 > '3142147557'
		ORDER BY e.telefono_acudiente_1";
	}
	//echo $sql_num;
    
    //$sql_num = "SELECT * FROM estudiantes WHERE n_documento IN ('999999', '9397454') ORDER BY telefono_acudiente_1";
    //$sql_num = "SELECT * FROM tbl_prueba ORDER BY telefono_acudiente_1";
	//echo "<br>".$sql_num; 
	...
	while ($fila_num = mysqli_fetch_array($res_num)){
        ...
        if($destino == $fila_num['telefono_acudiente_1']) {
			...
			//Se inserta en la tabla temp para control
			$sql_temp = "INSERTO INTO tbl_temp (c1, v1, C2) VALUES ('NA', 0, '".$fila_num['telefono_acudiente_1']."')";
			$res_temp = mysqli_query($conexion,$sql_temp);
            
            //Se valida si la imagen o el vídeo es local para subirla al servidor
        	if($envio == 1) {
        	    if($tipoimg == 1) {
                ...
                curl_setopt_array($curl, array(
                  ...
                  CURLOPT_POSTFIELDS => "token=".$token."&to=+57".$fila_num['telefono_acudiente_1']."&image=".$rutaimage."&caption=".$textoimg."&referenceId=&nocache=",
        	    ...
        	    if($tipoimg == 1) {
        	        $video = $_FILES['ImagenW']['name'];
                	$ruta = $_FILES['ImagenW']['tmp_name'];
                	$tipo_archivo = $_FILES['ImagenW']['type'];
                	$destino = "../../assets/videos/".$video;
                	copy($ruta, $destino);
                	//$imglocal = $_REQUEST['ImagenW'];
                	
                	$rutavideo = "https://unicab.org/assets/videos/".$video;
        	    }
        	    else if($tipoimg == 2) {
        	        $rutavideo = "https://unicab.org/assets/videos/".$vidservidor;
        	    }
                ...
                curl_setopt_array($curl, array(
                  ...
                  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                  CURLOPT_CUSTOMREQUEST => "POST",
                  CURLOPT_POSTFIELDS => "token=".$token."&to=+57".$fila_num['telefono_acudiente_1']."&video=".$rutavideo."&caption=".$textoimg."&referenceId=&nocache=",
                  CURLOPT_HTTPHEADER => array(
                    "content-type: application/x-www-form-urlencoded"
                  ),
                ));
				...
				//Se sube el pdf al servidor
        	    $pdf = $_FILES['pdfW']['name'];
				$ruta = $_FILES['pdfW']['tmp_name'];
				$tipo_archivo = $_FILES['pdfW']['type'];
				$destino = "../../assets/pdfs/whatsapp/".$pdf;
				copy($ruta, $destino);
				
				$rutapdf = "https://unicab.org/assets/pdfs/whatsapp/".$pdf;
                ...
                curl_setopt_array($curl, array(
                  ...
                  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
                  CURLOPT_CUSTOMREQUEST => "POST",
                  CURLOPT_POSTFIELDS => "token=".$token."&to=+57".$fila_num['telefono_acudiente_1']."&filename=".$pdf."&document=".$rutapdf."&caption=".$textopdf."&referenceId=",
                  CURLOPT_HTTPHEADER => array(
                    "content-type: application/x-www-form-urlencoded"
                  ),
                ));
            ...
            $valores = [$response,$fila_num['acudiente_1'],$fila_num['telefono_acudiente_1']];
					...
					<div class="form-grids row widget-shadow" data-example-id="basic-forms"> 
						...
						<div class="form-body" style="height: 500px; overflow: scroll;">
							<?php
							    echo "url: ".$url."<br>";
							    echo "envio: ".$envio."<br>";
							    echo "tipo recurso: ".$tipoimg."<br>";
							    echo "ruta recurso: ".$rutaimage."<br>";
